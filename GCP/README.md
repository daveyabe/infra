...
This directory contains scripts and a Makefile to provision a new GCP project end-to-end: create the project, link billing, enable APIs, create a Terraform service account, and set up Workload Identity Federation for GitHub Actions.
...
4. **Terraform service account**  
   `./04-SA-account-TF.sh <PROJECT_ID>`
...
- **Terraform:** Use the `terraform-pro@<PROJECT_ID>.iam.gserviceaccount.com` service account for Terraform (e.g. via `gcloud auth application-default login` as that user or by exporting keys if required).
- **GitHub Actions:** Step 5 prints `WORKLOAD_IDENTITY_PROVIDER` and `SERVICE_ACCOUNT`. Use them in your workflow with `google-github-actions/auth` and `workload_identity_provider` / `service_account` (see the script output for an example job snippet).

---

## Troubleshooting

- **“Service account terraform-pro doesn’t exist”** — Ensure step 3 (enable-apis) has run so the IAM API is enabled, then run step 4 again. The script creates the SA if missing and waits briefly for IAM propagation.
